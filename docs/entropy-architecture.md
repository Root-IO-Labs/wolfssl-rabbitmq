...
This document describes the entropy and random number generation (RNG) architecture for the RabbitMQ 3.13.7 FIPS-enabled container image. Proper entropy management is **critical** for FIPS 140-3 compliance, as all security-critical cryptographic operations (key generation, IV creation, nonce generation, etc.) must use approved RNG mechanisms.
...
### 5.1 RabbitMQ Cryptographic Operations

RabbitMQ uses cryptography for:

| Operation | FIPS Requirement | Implementation |
|-----------|------------------|----------------|
| **AMQP/AMQPS Connections (TLS)** | FIPS-approved algorithms | ✅ Via FIPS OpenSSL + wolfProvider |
| **Authentication (SCRAM-SHA-256)** | Salt generation must use approved RNG | ✅ Via OpenSSL RAND_bytes() → wolfSSL |
| **Message Encryption/Signing** | All crypto via FIPS module | ✅ Via Erlang crypto → OpenSSL API → wolfProvider |
| **Key Derivation (KDF)** | FIPS-approved KDF with approved RNG | ✅ Via wolfSSL FIPS |
| **Cookie Generation** | Erlang distribution cookie | ✅ Via Erlang crypto → wolfSSL FIPS |

**Key Configuration:**

Erlang is built with:
```
--with-ssl=/usr/local/openssl \
LDFLAGS="-L/usr/local/openssl/lib64 -Wl,-rpath=/usr/local/openssl/lib64"
```

This ensures Erlang's crypto module links to FIPS OpenSSL, which uses wolfProvider, which uses wolfSSL FIPS DRBG.
...
5. **Source Code Evidence:**
   - Erlang/RabbitMQ build flags (linking to FIPS OpenSSL)
   - `fips-startup-check.c` source code
6. **Runtime Configuration:**
   - OpenSSL configuration (`openssl.cnf`)
   - Environment variables (`OPENSSL_CONF`, `LD_LIBRARY_PATH`, `OPENSSL_MODULES`)
...
# Test 3: Test Erlang crypto functions
erl -noshell -eval 'Hash = crypto:hash(sha256, <<"test">>), io:format("~s~n", [binary:encode_hex(Hash, lowercase)]), halt().'

# Test 4: Test RabbitMQ FIPS validation
/usr/local/bin/test-rabbitmq-fips.sh

# Test 5: Verify no /dev/urandom usage for crypto operations
strace -e openat beam.smp 2>&1 | grep urandom
...
# Measure Erlang crypto performance
time erl -noshell -eval 'lists:foreach(fun(_) -> crypto:strong_rand_bytes(32) end, lists:seq(1, 10000)), halt().'
```
